from datetime import datetime
import cv2
import numpy as np
import pandas as pd
import pytesseract
from ultralytics import YOLO

from JsonFileFactory import JsonFileFactory
from RecordingNumberPlate import RecordingNumberPlate

# Assuming that pytesseract and YOLO model are properly set up
pytesseract.pytesseract.tesseract_cmd = r'C:\Program Files\Tesseract-OCR\tesseract.exe'
model = YOLO('../best.pt')

# Function to handle mouse events
def RGB(event, x, y, flags, param):
    if event == cv2.EVENT_MOUSEMOVE:
        point = [x, y]
        print(point)

cv2.namedWindow('RGB')
cv2.setMouseCallback('RGB', RGB)

cap = cv2.VideoCapture('../mycarplate.mp4')

with open("../coco1.txt", "r") as my_file:
    data = my_file.read()
class_list = data.split("\n")

area = [(27, 417), (16, 456), (1015, 451), (992, 417)]
count = 0
processed_numbers = set()

# Initialize JsonFileFactory and load existing data
jff = JsonFileFactory()
filename = "../dataset/car_plate_data.json"
plate_records = jff.read_data(filename, RecordingNumberPlate)

while True:
    ret, frame = cap.read()
    count += 1
    if count % 3 != 0:
        continue
    if not ret:
        break

    frame = cv2.resize(frame, (1020, 500))
    results = model.predict(frame)
    a = results[0].boxes.data
    px = pd.DataFrame(a).astype("float")

    for index, row in px.iterrows():
        x1 = int(row[0])
        y1 = int(row[1])
        x2 = int(row[2])
        y2 = int(row[3])

        d = int(row[5])
        c = class_list[d]
        cx = (x1 + x2) // 2
        cy = (y1 + y2) // 2

        result = cv2.pointPolygonTest(np.array(area, np.int32), (cx, cy), False)
        if result >= 0:
            crop = frame[y1:y2, x1:x2]
            gray = cv2.cvtColor(crop, cv2.COLOR_BGR2GRAY)
            gray = cv2.bilateralFilter(gray, 10, 20, 20)

            text = pytesseract.image_to_string(gray).strip()
            text = text.replace('(', '').replace(')', '').replace(',', '').replace(']', '')
            if text not in processed_numbers:
                processed_numbers.add(text)

                # Get current time and date
                current_time = datetime.now().strftime("%H:%M:%S")
                current_date = datetime.now().strftime("%Y-%m-%d")

                # Record the plate information
                plate_record = RecordingNumberPlate(numberplate=text, time=current_time, date=current_date)
                plate_records.append(plate_record)

                # Save the records to JSON file
                jff.write_data(plate_records, filename)

                # Draw the bounding box
                cv2.rectangle(frame, (x1, y1), (x2, y2), (0, 255, 0), 1)
                cv2.imshow('crop', crop)

    cv2.polylines(frame, [np.array(area, np.int32)], True, (255, 0, 0), 2)
    cv2.imshow("RGB", frame)

    if cv2.waitKey(1) & 0xFF == 27:
        break

cap.release()
cv2.destroyAllWindows()
